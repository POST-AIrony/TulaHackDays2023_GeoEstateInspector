...
def analyze_tiff(path_to_tif, path_to_model, path_to_land_map):
    unique_name = generate_unique_name(path_to_tif)
    image_processing(path_to_tif, path_to_model, unique_name)
    results = find_parcel_for_building(unique_name, path_to_land_map)
    write_results_to_pdf(results, unique_name)
    path_to_zip = archive_and_delete_files(unique_name)
    return path_to_zip

def analyze_photo(path_to_file, path_to_model, path_to_land_map):
    name = True # to do
